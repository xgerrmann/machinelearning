...
if(!submit){
	...
	for(i in 1:n_crossval){
		...
		#X_imputed = mice(X, m=1, maxit=1, method = 'pmm', seed = 500)
		X_tmp = mice(X)
		X_imputed = complete(X_tmp,1)
		#X_imputed = missForest(X)
		#print(head(X,10))
		#print(head(X_imputed,10))
		# split test and trainset
		print(dim(X_imputed))
		print(n_train)
		trainX <- X_imputed[1:n_train,]
		trainy <- y[1:n_train]
		testX <- X_imputed[(n_train+1):n_data,]
