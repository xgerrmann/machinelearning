...
if(!submit){
	...
	for(i in 1:n_crossval){
		...
		dy = dim(trainy)
		print(dy)
		model <- trainr( trainX, trainy, learningrate=lr, learningrate_decay = lr_d,
						momentum = moment, hidden_dim=hid_dim,numepochs=numepoch,
						start_from_end = st_f_end)
		#summary( model )
		
		#p <- predict( model, trainX, type="class" )
		#p <- predict( model, testX, type="class" )
		p <- predictr( model, testX, hidden = FALSE )
		#p
		#testy
		#length(trainy)
		#length(testy)
